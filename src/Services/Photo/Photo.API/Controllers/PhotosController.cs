...
namespace MyAlbum.Services.Photo.API.Controllers
{
    ...
    public class PhotosController : Controller
    {
            ...
            IWebHostEnvironment host, IObjectDetectionService objectDetectionService)
        {
            ...
            this.outputFolderUrl = appBaseUrl + "/uploads/output";
        }
        ...
        public async Task<IActionResult> GetPhoto([FromRoute] int id)
        {
            ...
            var photoResource = mapper.Map<Models.Photo, PhotoResource>(photo);
        ...
        {
            var filter = mapper.Map<PhotoQueryResource, Models.PhotoQuery>(filterResource);
            var photos = await this.photoRepository.GetPhotos(filter);
            var photoResources = mapper.Map<IEnumerable<Models.Photo>, IEnumerable<PhotoResource>>(photos);
